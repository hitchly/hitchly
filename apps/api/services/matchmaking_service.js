...
function calculateScheduleScore(riderTime, driverTime) {
...
function calculateLocationScore(detourSeconds, toleranceMinutes) {
...
function calculateComfortScore(current, max, requested) {
  if (current + requested > max) return 0.0;
  return 1.0 - current / max;
}
function calculateCompatibilityScore(riderPrefs, driverPrefs) {
...
function computeScore(weights, scores) {
  return (
...
export async function findMatchesForUser(request) {
  ...
  const existingPickupsMap = new Map();
  try {
  ...
  let tripRequestCountMap = new Map();
  if (tripIds.length > 0) {
  ...
  const candidates = allRides.map((row) => {
    ...
    return {
      ...
      driverEmail: row.driverEmail || row.user.email,
  ...
  const validScoredResults = scoredResults.filter((r) => r !== null);
  const minCost = Math.min(...validScoredResults.map((r) => r.cost));
  const finalMatches = validScoredResults.map(
    ({
      ride,
      cost,
      detourSeconds,
  ...
  const validMatches = finalMatches.filter((m) => m !== null);
 ...
 */
function generateDummyMatches(request) {
  const dummyDrivers = [
    {
      name: "Alex Johnson",
