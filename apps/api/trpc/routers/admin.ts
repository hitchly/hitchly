...
export const adminRouter = router({
  ...
  /**
   * Simulate driver completing all pickups and dropoffs for a trip
   * - accepted -> on_trip
   * - on_trip -> completed (in current order)
   * - complete trip if all requests completed
   */
  simulateDriverComplete: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (acceptedIds.length > 0) {
        ...
        updatedAccepted = acceptedIds.length;
      }
      ...
      if (onTripIds.length > 0) {
        await ctx.db
          ...
          .where(inArray(tripRequests.id, onTripIds));
        updatedOnTrip = onTripIds.length;
      }

      // Re-fetch to determine completion
      const postRequests = await ctx.db
        .select()
        .from(tripRequests)
        .where(eq(tripRequests.tripId, input.tripId));
      const anyIncomplete = postRequests.some(
        (r) =>
          r.status === "pending" ||
          r.status === "accepted" ||
          r.status === "on_trip"
      ...
      return {
        updatedAccepted,
        updatedOnTrip,
        tripCompleted: tripUpdated,
        totalRequests: postRequests.length,
      };
    }),
