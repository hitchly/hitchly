...
\begin{itemize}
    ...
    \item \textbf{UpdatedFields} --- Partial trip update input: each of origin (Location), destination (Location), departureTime ($\mathbb{R}$), and availableSeats ($\mathbb{N}$) may be present or omitted independently.
    ...
    \item \textbf{RequestData} = (pickup: Location, dropoff: Location, time: $\mathbb{R}$, seatsNeeded: $\mathbb{N}$) --- Ride request input
    ...
    \item \textbf{RatingData} = (tripId: $\mathbb{N}$, score: $\mathbb{R}$, comment: string) --- Rating submission input
    \item \textbf{RatingRecord} = (ratingId: $\mathbb{N}$, reviewerId: $\mathbb{N}$, revieweeId: $\mathbb{N}$, tripId: $\mathbb{N}$, score: $\mathbb{R}$, comment: string, timestamp: $\mathbb{R}$) --- Complete rating information
    \item \textbf{RatingList} = seq of RatingRecord --- Sequence of rating records
    \item \textbf{ReportData} = (reportedUserId: $\mathbb{N}$, tripId: $\mathbb{N}$, reason: string, description: string) --- Incident report input
    \item \textbf{ReportRecord} = (reportId: $\mathbb{N}$, reporterId: $\mathbb{N}$, reportedUserId: $\mathbb{N}$, tripId: $\mathbb{N}$, reason: string, description: string, status: string, timestamp: $\mathbb{R}$) --- Complete incident report
    \item \textbf{ReportList} = seq of ReportRecord --- Sequence of report records
    \item \textbf{PaymentRecord} = (paymentId: $\mathbb{N}$, userId: $\mathbb{N}$, tripId: $\mathbb{N}$, amount: $\mathbb{R}$, status: string, timestamp: $\mathbb{R}$) --- Transaction record
