...
\subsection{Module}
The Matching Module is the core algorithmic engine of the system. 
...
Its outputs drive the user interface, where users browse and select potential ride matches.

\paragraph{Frontend}
Implemented in the client application, the frontend presents matches as interactive cards in a sequential interface. 
Key features include:
\begin{itemize}
    \item Input gestures for accepting or passing on potential matches.
    \item Real-time display of top recommended drivers or riders with route, time, and compatibility score.
    \item API integration through remote procedure calls.
    \item Visual match indicators such as qualitative labels or percentage score badges.
    \item Immediate visual feedback for mutual matches.
\end{itemize}

\paragraph{Backend}
The backend implements the matchmaking logic within the application server, encapsulated in a dedicated service component. 
The design follows the Strategy Pattern to keep scoring algorithms modular. 
Primary responsibilities include:
\begin{itemize}
    \item Retrieving candidate users and associated trips from the persistence layer.
    \item Filtering incompatible pairs by hard constraints: verified institutional email, opposite roles, overlapping time windows, and nearby routes.
    ...
    \item Sorting and returning the highest-scoring results to the client.
    \item Storing selection actions and updating mutual matches.
\end{itemize}
The backend exposes endpoints for finding matches, submitting selections, and retrieving confirmed results.

\paragraph{Data}
The data layer maintains persistent records of matches, interactions, and scoring metrics within the relational database. 
Key entities include:
\begin{itemize}
    \item \texttt{MatchEntity} --- stores confirmed mutual matches (rider <-> driver) with timestamp and score.
    \item \texttt{InteractionEntity} --- records all user decisions (accept/pass) for analytics and recommender tuning.
    \item \texttt{MatchHistoryEntity} --- archives past matches for feedback and trust scoring.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Verified institutional email
...
\begin{itemize}
    ...
    \item \textbf{Client Interaction Layer} - renders and collects user selection input.
    \item \textbf{Notification Module} - notifies users upon mutual matches.
\end{itemize}
It is the computational bridge between trip listings and user engagement,\linebreak supporting the key matchmaking feature.
...
\midrule
\texttt{findMatchesForUser} & userId: $\mathbb{N}$, request: RequestData & RankedMatchList & NotFound \\
\texttt{requestRide} & rideId: $\mathbb{N}$, pickup: Location & Confirmation & NotFound, BadRequest \\
\texttt{acceptRequest} & requestId: $\mathbb{N}$ & Confirmation & NotFound, BadRequest, Forbidden \\
\texttt{cancelRequest} & requestId: $\mathbb{N}$ & Confirmation & NotFound, Forbidden \\
\texttt{getMatchResults} & userId: $\mathbb{N}$ & MatchSummary & None \\
\texttt{computeScore} & rider: UserRecord, driver: UserRecord & Score: $\mathbb{R}$ & AlgorithmError \\
\bottomrule
\end{tabular}
}
\end{center}
...
\begin{itemize}
    ...
    \item \texttt{CandidatePool} - Set of Candidate Records (drivers or riders)\linebreak (valid in states: QUERYING\_RIDES, VALIDATING\_CANDIDATES, COMPUTING\_ROUTES, SCORING\_CANDIDATES)
    ...
    \item \texttt{MatchResults} - ordered sequence of potential matches with scores\linebreak (valid in states: RANKING\_MATCHES, MATCH\_COMPLETE)
...
\begin{itemize}
    \item Active network connection for queries.
    \item Persistence layer connection for retrieving and updating match data.
    \item Access to external mapping service for route distance calculations.
\end{itemize}

\paragraph{Assumptions}
\begin{itemize}
    \item Each user has an active, verified profile with trip data available.
    \item Distance and time calculations are performed via a reliable geocoding service.
    \item Only verified institutional users participate in matching.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{exception:} If transition to MATCH\_ERROR occurs: NotFound\linebreak (when $\text{CandidatePool} = \emptyset$ or $\text{validCandidates} = \emptyset$ or $\text{validMatches} = \emptyset$).\linebreak Error recovery transitions to READY state.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{exception:} If transition to REQUEST\_ERROR occurs: NotFound (ride not found) or BadRequest (ride full).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{exception:} If transition to ACCEPT\_ERROR occurs: NotFound, BadRequest, or Forbidden.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{output:} Success confirmation.
    \item \textbf{exception:} If transition to CANCEL\_ERROR occurs: NotFound or Forbidden.
\end{itemize}
