...
\begin{itemize}
    ...
    \item \textbf{output:} Returns $\texttt{Profiles[userId]}$.
    \item \textbf{exception:} \texttt{NotFoundError} if $\texttt{userId} \notin \texttt{Profiles}$.
\end{itemize}

\noindent \texttt{updateUserProfile(userId, data)}:
\begin{itemize}
    \item \textbf{transition:} $\texttt{Profiles[userId]} := \texttt{Profiles[userId]} \cup \texttt{data}$ (fields in data overwrite existing).
    \item \textbf{output:} Returns updated $\texttt{Profiles[userId]}$.
    \item \textbf{exception:} \texttt{ValidationFailed} if $\texttt{validateProfileInput(data)} = \text{false}$.
\end{itemize}

\noindent \texttt{updatePreferences(userId, data)}:
\begin{itemize}
    \item \textbf{transition:} $\texttt{Preferences[userId]} := \texttt{data}$.
    \item \textbf{output:} Returns $\texttt{Preferences[userId]}$.
    \item \textbf{exception:} \texttt{ValidationFailed} if $\texttt{validatePreferenceInput(data)} = \text{false}$.
\end{itemize}

\noindent \texttt{deleteUser(userId)}:
\begin{itemize}
    \item \textbf{transition:} Removes \texttt{userId} from \texttt{Profiles}, \texttt{Preferences}, and \texttt{Vehicles}.
    \item \textbf{output:} \texttt{true}.
    \item \textbf{exception:} \texttt{UnauthorizedError} if $\text{requestorId} \neq \texttt{userId} \land \text{requestorRole} \neq \text{admin}$.
\end{itemize}
