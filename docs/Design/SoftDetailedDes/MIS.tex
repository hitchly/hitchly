...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\midrule
\texttt{init_payment()} & user_id, ride_id & Boolean (verified or unverified) & NotFoundError \\
\texttt{process_payment()} & payment_id, payment_method  & payment_status  & ExpiredSessionError \\
\texttt{get_paymentRecord()} & payment_id, user_id, ride_id & paymentRecord & NotFoundError \\
\bottomrule
\end{tabular}
...
\begin{itemize}
    \item \texttt{current_amount: float} — holds the current amount for the transaction.
    \item \texttt{session_token: string} — holds the current amount for the transaction.
    \item \texttt{sessionState: object} — holds session metadata for frontend persistence.
\end{itemize}
...
\begin{itemize}
    \item \texttt{db_connect(): bool} — This is to initialize a connection with the database service.
...
\begin{itemize}
    \item \texttt{validate_parameters(start_location, end_location, rider_count): bool} — Check if the values are not negative and are valid inputs.
\end{itemize}
...
\begin{itemize}
    \item \texttt{check_privilege(admin_ID, endpoint): bool} — This function validates admin privileges and returns a bool to indicate whether they have sufficient privileges or not.
\end{itemize}
