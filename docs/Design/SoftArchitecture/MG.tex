...
The decomposition follows the principle of information hiding, 
where each module conceals a design decision that may change independently.  
This structure improves maintainability, scalability, and adaptability of the system.  
The hierarchy is divided into three main categories: Hardware-Hiding, Behaviour-Hiding, and Software-Decision Modules.  
Only the leaf modules shown in Table~\ref{TblMH} are implemented.

\subsection{Hardware-Hiding Modules}
These modules provide the interface between Hitchly and the mobile device hardware.  
Each hides a specific hardware dependency that may vary between platforms.

\begin{itemize}
    \item \textbf{GPS Module:}  
    Provides access to real-time location data and coordinates for ride matching, route generation, and distance tracking.

    \item \textbf{Camera Module:}  
    Manages camera operations for capturing license and ID images.  
    Ensures proper image quality, orientation, and permission handling across devices.

    \item \textbf{Verification Module:}  
    Uses device sensors (e.g., camera) and external verification APIs to validate driver identity and license authenticity.
\end{itemize}

\subsection{Behaviour-Hiding Modules}
These modules define Hitchly’s externally visible behaviour as described in the SRS.  
Each hides a key design decision such as authentication, matching, or payment processing.  
Together, they implement all user-facing functionality and maintain a consistent interface across the system.

\begin{itemize}
    \item \textbf{Authentication Module:}  
    Handles McMaster SSO verification, account registration, and credential management.

    \item \textbf{Matching Module:}  
    Performs ride matching based on time, route, and preferences, and ranks recommended rides for users.

    \item \textbf{Mapping Module:}  
    Integrates with mapping APIs (Google Maps or Mapbox) to render routes, estimate distances, and calculate travel times.

    \item \textbf{Payment Module:}  
    Manages digital transactions, fare splitting, and payment verification through secure third-party APIs.

    \item \textbf{Database Module:}  
    Provides persistent storage for users, rides, and transaction history.  
    Handles schema management, queries, and indexing.

    \item \textbf{Notification Module:}  
    Sends push and in-app notifications for ride requests, confirmations, and reminders.

    \item \textbf{Rating Module:}  
    Manages rider and driver feedback, aggregates average scores, and updates user reputation.

    \item \textbf{Search Module:}  
    Enables users to search or filter available rides by location, date, and user preference.
\end{itemize}


\subsection{Software-Decision Modules}
These modules implement Hitchly’s internal computation and decision-making logic.  
They handle analytics, scheduling, safety enforcement, and pricing — the core business rules that support behaviour modules.

\begin{itemize}
    \item \textbf{Analytics Module:}  
    Collects and processes app usage and sustainability metrics to support decision-making and reporting.

    \item \textbf{Scheduling Module:}  
    Manages trip creation, availability, rescheduling, and cancellations for both riders and drivers.

    \item \textbf{Safety Module:}  
    Oversees incident reporting, user trust mechanisms, and enforces safety-related policies and verification results.

    \item \textbf{Pricing Module:}  
    Calculates ride fares and implements cost-sharing logic between participants based on route and number of passengers.
\end{itemize}
...
\toprule
\textbf{Level 1} & \textbf{Level 2 (Leaf Modules)}\\
\midrule
\multirow{3}{0.3\textwidth}{Hardware-Hiding Modules}
& GPS Module\\
& Camera Module\\
& Verification Module\\
\midrule
\multirow{8}{0.3\textwidth}{Behaviour-Hiding Modules} 
...
& Search Module\\
\midrule
\multirow{4}{0.3\textwidth}{Software-Decision Modules} 
& Analytics Module\\
& Scheduling Module\\
& Safety Module\\
& Pricing Module\\
...
The architecture follows a top-down dependency structure.  
Hardware-hiding modules interface directly with device sensors and platform APIs.  
Behaviour-hiding modules depend on these hardware interfaces to deliver user-facing features such as authentication, matching, and payments.  
The software-decision layer provides the computational and analytical foundation that supports higher-level behaviour.  
This separation of concerns allows independent modification of each component — for example, updating the pricing logic, integrating a new mapping API, or refining verification — without impacting the rest of the system.
