...
Only the leaf modules from Section 5 are included.

\subsection*{Hardware-Hiding Modules}

\subsubsection*{M1: GPS Module}
\textbf{Secrets:} How the app accesses and optimizes GPS data, including sampling rate, accuracy, and permission handling.  

\textbf{Services:} Provides real-time coordinates and continuous tracking used for routing, pricing, and ride matching.  

\textbf{Implemented By:} React Native Location API and Expo Location.  

\textbf{Type of Module:} Hardware-Hiding Module.

\subsubsection*{M2: Camera Module}
\textbf{Secrets:} Camera access configuration, resolution settings, and permission control.  

\textbf{Services:} Captures photos for license, ID, and profile verification; processes images for upload.  

\textbf{Implemented By:} Expo Camera API and React Native Permissions.  

\textbf{Type of Module:} Hardware-Hiding Module.

\subsubsection*{M3: Verification Module}
\textbf{Secrets:} Verification algorithms and third-party API credentials used for identity validation.  

\textbf{Services:} Validates user identity via McMaster SSO and license image verification.  

\textbf{Implemented By:} Node.js (Express) backend integrated with McMaster SSO OAuth API.  
...
\textbf{Secrets:} Token management and session lifetime configuration.  

\textbf{Services:} Handles user login, registration, and secure JWT refresh.  

\textbf{Implemented By:} Node.js + tRPC backend with PostgreSQL user store and JWT middleware.  

\textbf{Type of Module:} Behaviour-Hiding Module.

\subsubsection*{M5: Matching Module}
\textbf{Secrets:} Ride-matching algorithm and ranking heuristics based on route and time overlap.  

\textbf{Services:} Matches riders and drivers, returns ranked ride options, and records confirmed pairings.  

\textbf{Implemented By:} Node.js backend using Mapbox API and PostgreSQL.  

\textbf{Type of Module:} Behaviour-Hiding Module.

\subsubsection*{M6: Mapping Module}
\textbf{Secrets:} API key management and caching strategy for rendering maps and computing routes.  

\textbf{Services:} Displays maps, computes distances and ETAs, and provides coordinates for pricing and matching.  

\textbf{Implemented By:} React Native frontend using Google Maps / Mapbox SDK.  

\textbf{Type of Module:} Behaviour-Hiding Module.

\subsubsection*{M7: Payment Module}
\textbf{Secrets:} Payment flow design and validation logic for cost-sharing transactions.  

\textbf{Services:} Handles transactions, manages fare splitting, and stores payment records.  

\textbf{Implemented By:} Node.js backend integrated with a Stripe-like API and PostgreSQL.  

\textbf{Type of Module:} Behaviour-Hiding Module.

\subsubsection*{M8: Database Module}
\textbf{Secrets:} Schema definitions, indexing strategies, and ORM mappings.  

\textbf{Services:} Provides persistent data storage and structured query interfaces for all other modules.  

\textbf{Implemented By:} PostgreSQL database accessed via Drizzle ORM in Node.js backend.  

\textbf{Type of Module:} Behaviour-Hiding Module.

\subsubsection*{M9: Notification Module}
\textbf{Secrets:} Message queue structure and token-based delivery mechanism for push notifications.  

\textbf{Services:} Sends trip confirmations, cancellations, and safety alerts through the Expo Push Notification Service.  

\textbf{Implemented By:} Expo Push Service API with backend dispatcher in Node.js.  

\textbf{Type of Module:} Behaviour-Hiding Module.

\subsubsection*{M10: Rating Module}
\textbf{Secrets:} Scoring and weighting logic for user reputation computation.  

\textbf{Services:} Records ratings, aggregates averages, and updates driver and rider reputations.  

\textbf{Implemented By:} Node.js microservice with PostgreSQL + Drizzle ORM.  

\textbf{Type of Module:} Behaviour-Hiding Module.

\subsubsection*{M11: Search Module}
\textbf{Secrets:} Search indexing and filtering logic for ride listings.  

\textbf{Services:} Enables users to search rides by date, location, and preference filters.  

\textbf{Implemented By:} Node.js backend using PostgreSQL full-text search.  
...
\textbf{Secrets:} Data-aggregation and metric-computation methods for sustainability and usage tracking.  

\textbf{Services:} Collects anonymized usage data and generates reports for performance monitoring.  

\textbf{Implemented By:} Backend analytics engine using Node.js and PostgreSQL scheduled jobs.  

\textbf{Type of Module:} Software-Decision Module.

\subsubsection*{M13: Scheduling Module}
\textbf{Secrets:} Trip allocation and time-slot management algorithm.  

\textbf{Services:} Manages ride creation, modifications, and cancellations while preventing overlap conflicts.  

\textbf{Implemented By:} Node.js backend with Drizzle ORM transactional logic.  

\textbf{Type of Module:} Software-Decision Module.

\subsubsection*{M14: Safety Module}
\textbf{Secrets:} Safety policy enforcement logic and verification thresholds.  

\textbf{Services:} Oversees ride safety checks, monitors incidents, and handles emergency triggers.  

\textbf{Implemented By:} Node.js backend integrated with Verification and Notification modules.  

\textbf{Type of Module:} Software-Decision Module.

\subsubsection*{M15: Pricing Module}
\textbf{Secrets:} Fare-calculation formula and cost-sharing model parameters.  

\textbf{Services:} Computes ride prices and divides costs among passengers based on route and occupancy.  

\textbf{Implemented By:} Node.js backend using Mapping and Scheduling data.  
