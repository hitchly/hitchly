...
% ---------- Title page ----------
\title{Software Requirements Specification Document\\\progname}
...
\begin{table}[hp]
  ...
  April 2nd & Burhanuddin Kharodawala & Updated the document with TA/peer feedback\\
...
New processes, or improvements to existing processes, made possible by the project's results. It presents the business benefits expected from successful execution of the project. This chapter is the core of the Goals book, describing what the organization expects from the system [Meyer, 2022].
...
\begin{figure}[htbp]
  ...
  \includegraphics[width=1.0\textwidth]{goal_model.png}
...
\begin{figure}[htbp]
  \centering
  % path is relative to the .tex file; create docs/SRS/images/ and put your file there
  \includegraphics[width=1.1\textwidth]{use-case.png}
...
\begin{figure}[h] 
    \centering
    \includegraphics[width=0.95\linewidth]{Domain Model.png}
    \caption{Domain Model}
    \label{fig:domain-model}
\end{figure}

\subsection{(E.2) Symbolic Constants}
\begin{center}
\small
\begin{tabularx}{\textwidth}{|l|c|X|}
\hline
\textbf{Constant} & \textbf{Value} & \textbf{Meaning} \\ \hline
\texttt{C\_MATCH\_MAX\_SEC} & 10 & The maximum time it takes to match riders with drivers. \\ \hline
\texttt{C\_CONFIRMATION\_MAX\_SEC} & 5 & The maximum time it takes to send confirmation messages in seconds. \\ \hline
\texttt{C\_PROFILE\_LOAD\_SEC} & 2 & The maximum time it takes to load profile screen data in seconds. \\ \hline
\texttt{C\_PAYMENT\_PROCESS\_SEC} & 10 & The maximum time it takes to complete payment processing in seconds. \\ \hline
\texttt{C\_AUDIT\_RETENTION\_YEARS} & 1 & The minimum retention period in years for verification audit logs. \\ \hline
\end{tabularx}
\end{center}
These constants are justified and enforceable through the fit criteria defined in Section (S.5.1), using percentile-based performance measurements under representative load.


\subsection{(E.3) Components}
\begin{enumerate}
...
\item \textbf{Regulatory Constraint:} The project is constrained to comply with the PIPEDA laws and standards [Government of Canada, 2000]. This ensures that the user's data is handled carefully and with significant attention to maintaining data integrity and security. Each driver must have a Candian issued G2/G license, and the vehicle must be registered and insured. Each driver must comply with Ontario traffic rules and regulations [Government of Ontario, 1990]. They must also not exceed the maximum passenger limit for their vehicle.
\item \textbf{Coding Standards:} Any developement made for this project must follow a strict TypeScript standard, enforced with ESLint rules and Prettier formatting checks in the CI/CD checks during branch merges. [typescript-eslint, 2024] [Prettier, 2024]. This ensures good quality code, and maintainability of the codebase. 
\item \textbf{Payment Security Standards:} Any developement related to payment must allign with the PCI DSS requirements, as that data must not be stored directly in the application [PCI SSC, 2024].
\item \textbf{Accessibility Constraint:} All UI whithin the application must also satisfy WCAG 2.2 Level AA accessibility requirements. This ensures the accessibility of the application [W3C, 2023].
...
\begin{enumerate}
  \item \textbf{F211} Route and Schedule matching: The system shall compare driver routes and passenger locations to provide optimal ride matches.
  \item \textbf{F212} Availability Updates: The engine shall update available rides in real time as users create, edit, or cancel rides.
  \item \textbf{F213} Preference filtering: The engine shall allow users to filter matches by preferences, such as gender, music, vehicle type, etc.
  \item \textbf{F214} Multi-passenger support: The system shall allow for multiple passengers to match to the same driver, based on driver preference and vehicle capacity.
  \item \textbf{F215} No-match handling: If the system fails to find a ride match, the systems clearly displays that no matches have been found and provides alternate matches with nearby options.
\end{enumerate}
Non-functional requirements:
\begin{enumerate}
  \item \textbf{NF211} Matching performance: The engine shall generate ride matches within \texttt{C\_MATCH\_LATENCY\_SEC} seconds of a request.
  \item \textbf{NF212} Location security: The engine shall ensure that user’s information used for matchmaking is not accessible outside of the development environment.
\end{enumerate}
\vspace{1em}
\noindent \textbf{Scheduling and Booking Manager} \\ 
Functional Requirements:
\begin{enumerate}
  \item \textbf{F221} Ride scheduling: Users shall be able to schedule rides in advance based on the results of the matchmaking engine.
  \item \textbf{F222} Booking confirmation: The system shall generate confirmations for bookings in the applications, and notify passenger(s) and the driver.
  \item \textbf{F223} Cancellation and Rescheduling: Users shall be able to cancel or reschedule rides within the specified timeframe and rules.
  \item \textbf{F224} Reminder notifications: The system shall send reminders to all passengers and the driver before any scheduled rides.
\end{enumerate} 
Non-functional requirements:
\begin{enumerate}
  \item \textbf{NF221} Data Integrity: All booking data shall be securely stored and updated as required.
  \item \textbf{NF222} Confirmation speed: Booking confirmations and changes shall be finalized and delivered to users within \texttt{C\_CONFIRMATION\_LATENCY\_SEC} seconds.
\end{enumerate}
\vspace{1em}
\noindent \textbf{Safety and Verification Module} \\
Functional Requirements:
\begin{enumerate}
  \item \textbf{F231} User Identity Verification: The system shall validate users through university email authentication in order to use the application.
  \item \textbf{F232} Driver Verification: The system shall store and verify driver license and vehicle registration information.
  \item \textbf{F233} Ride Check-in: The system shall prompt passengers and drivers to check in at the start and end of the ride.
  \item \textbf{F234} Safety check: The system shall keep track of any misconduct reported by users or any strikes on a user’s account.
\end{enumerate}
Non-functional Requirements:
\begin{enumerate}
  \item \textbf{NF231} Data Security: Data used for verification shall be encrypted to ensure security.
  \item \textbf{NF232} Auditability: The system shall maintain a log of all verification actions for at least 1 year.
\end{enumerate}
\vspace{1em}
\noindent \textbf{Payment and Cost Sharing System} \\
Functional Requirements:
\begin{enumerate}
  \item \textbf{F241} Fare Calculation: The system shall automatically calculate the costs for the ride for each passenger based on distance, number of passengers, parking costs, etc.
  \item \textbf{F242} Secure Transactions: The system shall support payment in the form of credit card, debit card, and other mobile wallet payment methods.
  \item \textbf{F243} Hold on Funds: The system shall put funds on hold until after a ride has been completed, in order to deal with cancellations or refunds.
  \item \textbf{F244} Expense Summaries: The system shall provide ride expense summaries and earnings reports for drivers.
\end{enumerate}
Non-functional Requirements:
\begin{enumerate}
  \item \textbf{NF241} Transaction Security: Payments must be processed following security standards.
  \item \textbf{NF242} Processing Speed: Transactions shall complete within \texttt{C\_PAYMENT\_PROCESS\_SEC} seconds of initiation.
\end{enumerate}
\vspace{1em}
\noindent \textbf{User Profile Manager} \\
Functional Requirements:
\begin{enumerate}
  \item \textbf{F251} Profile Creation: The system shall allow users to create profiles when signing up, including personal details and ride preferences.
  \item \textbf{F252} Profile Updates: The system shall allow users to update their profile information at any time.
  \item \textbf{F253} Ride History: The system shall store and display ride history and ratings for drivers and passengers.
  \item \textbf{F254} View Profiles: The system shall allow users to view profiles of other users when matched with them.
  \item \textbf{F255} User-initiated account deletion: The system shall allow users to request deletion of their account and personal profile data using in-app settings.
  \item \textbf{F256} Deactivation on affiliation loss: The system shall automatically deactivate a user's account if they are no longer affiliated with McMaster. This would prevent them from using Hitchly services until they are re-verified.
\end{enumerate}
Non-functional Requirements:
\begin{enumerate}
  \item \textbf{NF251} Privacy: Any personal information used when signing up shall not be visible to other users.
  \item \textbf{NF252} Performance: Profile data shall load within \texttt{C\_PROFILE\_LOAD\_SEC} seconds.
\end{enumerate}

\noindent \textbf{Rating and Reputation System} \\
Functional Requirements:
\begin{enumerate}
  \item \textbf{F261} Ride Ratings: The system shall allow users to rate each other after each ride.
  \item \textbf{F262} Issue Reporting: The system shall allow users to report any issues or misconduct that occurs during a ride.
  \item \textbf{F263} Minimum Reputation Thresholds: The system shall enforce a minimum rating for certain actions (e.g: Offering rides, hosting multiple passengers, joining scheduled rides, etc.).
  \item \textbf{F264} Positive Reinforcement: The system shall reward users who consistently obtain high ratings by giving out badges, titles, and more.
\end{enumerate}
Non-functional Requirements:
\begin{enumerate}
  \item \textbf{NF261} Rating Integrity: The system shall detect and flag fradulent and spam reviews. 
\end{enumerate}
...
\end{center}
  ...
  F255 & User-initiated Account Deletion & Must Have & Essential for user privacy and data control. \\ \hline
  F256 & Deactivation on Affiliation Loss & Must Have & Ensures McMaster affiliation requirements to maintain reliability and trust.\\ \hline
...
\end{center}
  \vspace{2em}
% reflect the requirement ID to the functional non functional requirements in the previous section. For instance, F251 is the requirement ID for Profile Creation, F252 is the requirement ID for Profile Updates, and so on.
...
\begin{itemize}
  ...
  \item UI components are accessible (WCAG 2.2 Level AA compliance) [W3C, 2023].
...
\begin{itemize}
  \item ESLint and Prettier pass on all commits, ensuring consistent coding standards amongst the team [typescript-eslint, 2024] [Prettier, 2024].
    \item TypeScript compiler passes with no errors.
  \item Payment integration follows PCI DSS requirements [PCI SSC, 2024].
\end{itemize}
...
\textbf{Sprint 1: Project Foundations (October 1--31, 2025)}
\begin{itemize}
...
\textbf{Sprint 2: Create MVP (November 1--30, 2025)}
\begin{itemize}
...
\textbf{Sprint 3: Core System Expansion (December 1--31, 2025)}
\begin{itemize}
...
\textbf{Sprint 4: Profile Addition (January 1--31, 2026)}
\begin{itemize}
...
\textbf{Sprint 5: Final Feature Additions (February 1--28, 2026)}
\begin{itemize}
...
\textbf{Sprint 6: Testing and Finalization (March 1--April 10, 2026)}
\begin{itemize}
...
\subsection{(P.6) Likely and Unlikely Changes}
\textbf{Likely to change}
\begin{itemize}
  \item Parameters and weights associated with those in F211/F213 are likely to change after usability testing and feedback.
  \item No current favorite "driver/rider" selection in F215/F225 but it is likely to evolve based on rider feedback.
  \item Cost calculation used in F241 may change as fuel/parking conditions accross cities change. 
\end{itemize}

\textbf{Unlikely to change}
\begin{itemize}
  \item Verification through the McMaster email (F231, F256) is unlikely to change for the scope of this capstone.
  \item Privacy standards and code quality standards previously mentioned in the document (NF231, NF232, NF251) are unlikely to change due to regulatory constraints.
  \item Accessibility standards (WCAG 2.2 AA) and payment-security standards whithin the code (PCI DSS) are unlikely to change due to regulatory requirements.
\end{itemize}
...
Other helpful courses include SFWRENG 2AA4, SFWRENG 3BB4 (Software Design I and II) as they gave experience working in a group on a development project, and learning key software engineering principles which will be helpful
...
\nocite{Kebirungi2024,ChandaAndSo2025,PIPEDA2000,OntarioHTA1990,TSLintStandard2024,PrettierStandard2024,PCIDSS401,WCAG22}
