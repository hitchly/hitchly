...
Users often find it difficult to find a safe and reliable platform that connects riders with drivers to carpool. Hitchly aims to achieve this goal by implementing multiple layers of security and safety features which not only increase the user's trust in the application, but also make this a reliable platform for them. First off, only McMaster students, staff and faculty members can use the application. This will ensure that all users are verified through their university email. Moreover, drivers will have to add their vehicle and license details which will be verified by the application. This will ensure that all drivers are verified and credible. This goal can be measured by checking the number of users who use the application for carpooling in a given period.

\item \textbf{Increase Affordability} \\
Carpooling creates an opportunity for users to share the cost of commuting to and from university. This applies for both, riders and drivers. This can significantly help users to manage their finances and help them save their money. This goal can be measured by comparing the monthly cost of carpooling with the average monthly cost of commuting.
...
\item \textbf{Advanced Matching Algorithm} \\
Hitchly aims to implement an advanced matching algorithm that not only matches users based on their selected time/location, but also based on their preferences. For instance, if someone prefers a driver that is talkative or someone that listens to music, etc. This will allow users to have a more personalized expereince and increase user satisfaction. This goal can be measure by checking the number of matches that are hevaily based on user preferences in a given period.
\end{enumerate}
...
activities:
\begin{itemize}
\item In-App Tutorial: This extra will allow us to create a tutorial within the app to help users understand how to use the app and its features. This will enhance the user experience and make it easier for new users to navigate the app. This can be measured by tracking the number of users who complete the tutorial and their feedback on its usefulness.
...
\begin{enumerate}
    ...
    Burhan:\\
    Our team was able to effectively delegate the work for this deliverable and help each other out with the details. Moreover, this document set a good basis for our future deliverables and made sure that we are all on track and on the same page as a team. 
\\  Sarim:\\
    ...
    Burhan:\\
    Since we don;t have a dedicated supervisor for our project, it was initially a little difficult to finalize our main idea and confirm our goals. However, with the help of our stakeholders, we were able to verify that our idea is promising.\\
