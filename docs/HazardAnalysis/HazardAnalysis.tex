...
April 2nd, 2025 & Burhanuddin Kharodawala & Updated the document with TA/peer feedback\\
\bottomrule
\end{tabularx}
\end{table}
...
\noindent \textbf{CA-1:} The Maps API provides up-to-date navigation for all users in the surrounding GTA region.\\

\noindent \textbf{CA-2:} The application will only target McMaster students for the initial build.\\

\noindent \textbf{CA-3:} The expected user base of the application will be within the planned demographic (students and university faculty).\\

\noindent \textbf{CA-4:} API rate limits will not be exceeded under the expected user load.\\
...
5.3 & Matching Engine & Incorrect ride matches, no match & Algorithm error; timetable parsing bug; corrupted location input & Riders matched incorrectly; missed/unsafe rides & Backend logs; user feedback reports & Unit/integration testing; fallback to safe “no match” state (SRS S.6 Matching Engine) \\ \hline
...
5.11 & Payment Transactions (future) & Failed or incorrect charges & Payment gateway failure; API failure & Financial loss; user dissatisfaction & Transaction logs; strict testing & Add dispute option, allow manual override through admin portal (SRS S.3.2 Payment API) \\ \hline
